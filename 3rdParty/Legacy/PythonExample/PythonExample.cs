...
namespace Legacy.PythonExample
{
	internal class PythonExample : IPlugin
	{
		...
		private static bool _isScriptManagerInitialized;

		public void InitializeScriptManager()
		{
			if (_isScriptManagerInitialized)
				return;
			...
			foreach (var inst in ThirdPartyLoader.Instances)
			{
				ns.Add(inst.Name);
			}

			_scriptManager.Initialize(null, ns);

			_isScriptManagerInitialized = true;
		}
	}
}